...
./gradlew :ru-bizgen-plugin:docsScreenshots -Pmarketplace                        # + кадры 2560x1600 под витрину
...
С флагом `-Pmarketplace` к README-кадрам добавляются `marketplace-generators.png` и
`marketplace-settings.png` — фиксированная канва 1280×800 (16:10) в ×2, то есть ровно 2560×1600,
как рекомендует JetBrains. Кадр центрируется на попапе/диалоге и прижимается к границам нижнего слоя,
чтобы в него не попала пустая заливка; попап выше 800 px, поэтому в витринном кадре он показан
с прокруткой — так его и видит пользователь. Панели инструментов перед съёмкой скрываются
(`HideAllWindows`), поэтому в кадр не попадают дерево проекта, тулбар и бейдж лицензии тестовой IDE.
...
**Размеры для Marketplace и IDE.** Официальные требования JetBrains:

| Что | Требование | Источник |
|-----|-----------|----------|
| Скриншот | **1280×800 (16:10)** — рекомендуемый; минимум **1200×760** | [Approval Guidelines](https://plugins.jetbrains.com/docs/marketplace/jetbrains-marketplace-approval-guidelines.html), [Best practices](https://plugins.jetbrains.com/docs/marketplace/best-practices-for-listing.html) |
| Логотип | **40×40, SVG**, ≤ 2–3 КБ, `META-INF/pluginIcon.svg` (+ опциональный `pluginIcon_dark.svg`); показывается 40×40 в списке плагинов и 80×80 на странице | [Plugin Icon File](https://plugins.jetbrains.com/docs/intellij/plugin-icon-file.html) |
| Видео | ссылка на YouTube, промо/демо **< 5 минут** | Best practices |
| Картинки в описании | ориентир по ширине **600–800 px** | Best practices |
| Запрещено | фото экрана и рамки устройств, мелкий нечитаемый текст, реклама и контент не по теме плагина | Approval Guidelines |

Отдельных числовых требований к GIF у JetBrains нет — формат просто разрешён.

**Практика для Ru BizGen** (следствия из требований выше, не цитаты):

- снимать и записывать на канве **1280×800**, отдавать в ×2 — **2560×1600**: на HiDPI не мылит,
  а при даунскейле до 1280×800 кириллица остаётся читаемой;
- **GIF писать в тех же 1280×800** и не масштабировать окно при записи — растянутый кадр даёт
  замыленный текст, а это прямой пункт запрета («мелкий нечитаемый текст»). Текущие `main.gif`
  и `work-sample.gif` сняты 1440×1024 (соотношение 1.41 вместо 1.60) — следующую запись делать
  в 1280×800; ориентиры: ≤ 20 секунд, 12–15 fps, до нескольких МБ (сейчас 0,9 и 2,1 МБ);
- **попап целиком в 800 px по высоте не помещается**: 31 генератор дают ~974 логических пикселя.
  Для Marketplace либо кадр показывает попап со скроллом (так его и видит пользователь), либо
  съёмка идёт на уменьшенном шрифте редактора. Полный список без скролла — это README-кадр
  из `docsScreenshots`, у него своя рамка;
- первым в галерее ставить кадр, который объясняет плагин без подписи, — он же идёт на карточку;
- в IDE список плагинов показывает только логотип и название, медиа видно на странице плагина.

**Проверить в репозитории:** у `META-INF/pluginIcon.svg` проставлены `width="512" height="512"`
при `viewBox="0 0 512 512"`. Рендерится он верно, но требование JetBrains — одна размерность 40×40;
безопаснее выставить `width="40" height="40"`, оставив `viewBox` как есть.
