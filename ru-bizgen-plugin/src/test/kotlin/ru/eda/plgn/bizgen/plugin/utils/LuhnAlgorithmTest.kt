...
internal class LuhnAlgorithmTest : BaseTest() {
  ...
  inner class CalculateCheckDigitCases {
    ...
    internal fun `Should return correct check digit`() = tests(VALID_NUMBERS, { "Card number: $it" }) { cardNumber ->
      ...
      // when
      val calculatedCheckDigit = LuhnAlgorithm.calculateCheckDigit(sourcePayload)
    ...
    internal fun `Should return exception when length of source is not odd`() {
      // expect
      shouldThrow<IllegalArgumentException> { LuhnAlgorithm.calculateCheckDigit("1111111111111111") } shouldHaveMessage "The source: 1111111111111111 (length = 16) must have an odd length"
    }
  }
  ...
  inner class IsValidCases {
    ...
    internal fun `Should return card number is invalid`() = tests(INVALID_NUMBERS, { "Card number: $it is invalid" }) { cardNumber ->
      // expect
      LuhnAlgorithm.isValid(cardNumber) shouldBe false
    }

    @Test
    internal fun `Should return exception when length of source is not even`() {
      // expect
      shouldThrow<IllegalArgumentException> { LuhnAlgorithm.isValid("111111111111111") } shouldHaveMessage "The source: 111111111111111 (length = 15) must have an even length"
    }
  }
