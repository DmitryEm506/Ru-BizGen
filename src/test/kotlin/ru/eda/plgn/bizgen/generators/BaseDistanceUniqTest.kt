 ...
 */
internal abstract class Percentile95DistanceUniqStrTest : Percentile95DistanceUniqTest<String> {

  internal constructor(generator: Generator<String>, category: UniqDistanceCategory) : super(generator, category)

  constructor(generator: Generator<String>, distanceLimit: IntRange, minimalUniqueDistance: Int) : super(
    generator,
    distanceLimit,
    minimalUniqueDistance
  )
}
...
internal abstract class Percentile95DistanceUniqTest<T : Any>(
  ...
  protected val distanceLimit: IntRange,
  protected val minimalUniqueDistance: Int,
) : PercentileDistanceUniqTest<T>(Percentile.PERCENTILE_95, generator) {

  constructor(generator: Generator<T>, category: UniqDistanceCategory) : this(
    generator,
    category.distanceLimit,
    category.minimalUniqueDistance
  )
...
/**
 * Категории уникальности дистанции.
 *
 * @property distanceLimit интервал, на котором происходит поиск лучшей дистанции уникальности
 * @property minimalUniqueDistance ожидаемая дистанция. Она может быть и меньше, чем максимально найденная дистанции
 */
internal enum class UniqDistanceCategory(val distanceLimit: IntRange, val minimalUniqueDistance: Int) {
  /**
   * Приемлемая дистанция уникальности.
   * - Поиск от 30 до 50. Найденная дистанция должна быть не хуже 40
   */
  NORMAL(distanceLimit = 30..50, minimalUniqueDistance = 40),

  /**
   * Отличная дистанция уникальности.
   * - Поиск от 100 до 130. Найденная дистанция должна быть не хуже 110
   */
  HIGH(distanceLimit = 100..130, minimalUniqueDistance = 110),
}
